    ...
    <section className='home destinations'>
      <div className="px-5 pt-32 grid grid-cols-1 gap-5 md:grid-cols-2  xl:max-w-7xl xl:mx-auto">
        <article className='text-center md:text-left'>
          <h2 className='text-white text-3xl font-extralight uppercase tracking-wider pb-10'>01</h2>
          ...
          <ul className='flex items-center justify-center md:justify-start mb-10'>
            {planets.map((item,index) =>(
                <li key={index} className='mr-5'>
                  ...
                  className={`uppercase font-semibold tracking-wider text-gray-200 pb-2 border-b border-transparent ${index === value && "border-b border-white"}`}
                  >
                    {item.name}
                  </button>
                </li>
